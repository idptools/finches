...
VALID_AA = ['M', 'G', 'K', 'T', 'Y', 'A', 'D', 'E', 'V', 'L', 'Q', 'W', 'R', 'F', 'S', 'H', 'N', 'P', 'C', 'I']
...
class CALVADOS_model(ForcefieldModel):
    ...
    def __init__(self, version='CALVADOS2', salt=0.150, pH=7.4, temperature=288, input_directory='default'):
        """
        ...
        if not exists(f'{data_prefix}/calvados_residues_dict.pickle'):
            raise Exception(f'Using [{data_prefix}] as our data directory but no calvados_residues_dict.pickle file found')
    ...
    def _genParams(self):
        ...
        self._residue_params['H']['q'] = 1. / (1 + 10**(self.pH - 6))
    ...
    def _build_pairwise_maps(self):
        ...
        for r1 in VALID_AA:
            ...
            for r2 in VALID_AA:
                ...
                self.SIGMA_ALL[r1][r2] = (self._residue_params[r1]['sigmas'] + 
                                          self._residue_params[r2]['sigmas']) / 2

                # Average lambda: (lambda_r1 + lambda_r2) / 2
                self.LAMBDA_ALL[r1][r2] = (self._residue_params[r1]['lambdas'] + 
                                           self._residue_params[r2]['lambdas']) / 2

                # Yukawa epsilon: q1 * q2 * lB * RT
                self.YUKAWA_EPS_ALL[r1][r2] = (self._residue_params[r1]['q'] * 
    ...
    def compute_ashbaugh_hatch(self, residue_1, residue_2, r):
        ...
        l = self.LAMBDA_ALL[residue_1][residue_2]

        return ashbaugh_hatch(r * 0.1, s, l, cutoff=self.cutoff, lj_eps=self.lj_eps)
    ...
    def compute_full_calvados(self, residue_1, residue_2, r):
        ...
        energies = compute_calvados_energy(
            r_nm, s, l, q, self.yukawa_kappa,
            cutoff=self.cutoff, lj_eps=self.lj_eps, yukawa_r_cut=self.yukawa_r_cut
        )
...
# .....................................................................................
#
def ashbaugh_hatch(r, s, l, cutoff=2.0, lj_eps=4.184*0.2):
    """
    ...
    cutoff : float
        Cutoff distance multiplier (default 2.0)
    ...
    # y = 4*eps*l*((s/r)^12 - (s/r)^6 - shift)  -- attractive regime
    # z = 4*eps*((s/r)^12 - (s/r)^6 - l*shift) + eps*(1-l)  -- repulsive regime
    y = 4*eps*l*(s_over_r_12 - s_over_r_6 - shift)
    z = 4*eps*(s_over_r_12 - s_over_r_6 - l*shift) + eps*(1-l)
...
def compute_calvados_energy(r, s, l, q, yukawa_kappa, cutoff=2.0, lj_eps=4.184*0.2, yukawa_r_cut=4.0):
    ...
    """
    ah = ashbaugh_hatch(r, s, l, cutoff=cutoff, lj_eps=lj_eps)
