...
class ForcefieldModel(ABC):
    ...
    def __init__(
        ...
        dielectric : float
            Dielectric constant of the solvent (e.g. 80.0 for water).
        ...
        all_residues_types : list of lists
            Nested list defining valid residue groupings, where each sublist
            contains residues allowed to occur together in a sequence.

        conditions : list
            List of tunable condition names; some models will allow you to
            vary environmental conditions, and this list defines which of
    ...
    def _get_default_distance_array(
        self, start: float = 0.1, stop: float = 30.0, step: float = 0.01
    ) -> np.ndarray:
        """
    ...
    def validate_residue_pair(self, residue_1: str, residue_2: str) -> None:
        ...
        if not self.is_valid_residue(residue_1):
            raise ValueError(
                f"Residue '{residue_1}' is not valid for {self.__class__.__name__}"
            )
        if not self.is_valid_residue(residue_2):
            raise ValueError(
                f"Residue '{residue_2}' is not valid for {self.__class__.__name__}"
            )
